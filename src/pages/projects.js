...
const Projects = () => {
    const data = useStaticQuery(
        graphql`
        query {
            allMarkdownRemark(sort: {fields: frontmatter___date, order: DESC}, filter: {frontmatter: {project: {eq: true}}}) {
              edges {
                node {
                  frontmatter {
                    title
                            ...
                            <div className="flex flex-wrap md:mt-32">
                                <div className="h-screen w-full grid grid-flow-row md:grid-flow-col auto-rows-max">
                                    ...
                                    <span className="md:mt-10 -mt-10">
                                        <h2 className="md:text-4xl text-base">{edge.node.frontmatter.title}</h2>
                                        <p className="md:text-base text-xs mb-2">{edge.node.frontmatter.tags}</p>
